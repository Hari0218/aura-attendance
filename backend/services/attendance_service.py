...
async def register_student_face(db: AsyncIOMotorDatabase, student_id: str, image_bytes: bytes):
    """
    Process a student face image, extract an ArcFace embedding, and store it.

    STRICT MODE: The full-image fallback has been permanently removed.
    A real face MUST be detected in the photo, otherwise the image is rejected.
    This prevents garbage 'room-background' embeddings that cause false positives.
    """
    ...
    if not faces:
        # NO fallback. Force the user to upload a better photo.
        logger.warning(f"No face detected in registration photo for student {student_id}. Photo rejected.")
        return False, (
            "No face detected in this photo. Please upload a clear, well-lit, "
            "frontal face photo where your face takes up most of the frame. "
            "Avoid side profiles, sunglasses, or photos taken far away."
        )
    ...
    embedding = face_system.extract_embeddings(face_image=face_image)
    if embedding is None:
        return False, "AI failed to extract a face signature. Please try a different, clearer photo."
    ...
    existing_count = await db.face_embeddings.count_documents({"student_id": student_id})
    return True, f"Face registered successfully ({existing_count} photo(s) stored for this student)"
...
async def process_attendance(db: AsyncIOMotorDatabase, classroom_image_bytes: bytes, class_id: str = None, period: str = None):
    """
    Process classroom photo, identify students, and return results.

    IMPORTANT: This function NO LONGER writes to the database automatically.
    Attendance is only persisted when the teacher explicitly clicks "Confirm & Save"
    (the /finalize endpoint). This prevents stale wrong records from accumulating.
    """
    ...
    faces = face_system.detect_faces(image)
    if not faces:
        return {"recognized_students": [], "absent_students": [], "confidence_scores": [], "unknown_faces_count": 0}, "No faces detected in the image"
    ...
    for i, face in enumerate(faces):
        ...
        if student_id != "Unknown":
            # Filter to only students in the selected class
            if class_id and student_map.get(student_id, {}).get("class_id") != class_id:
                unknown_count += 1
                continue
            # Keep highest confidence per student (deduplicate)
            if student_id not in best_matches or confidence > best_matches[student_id]:
                best_matches[student_id] = confidence
        else:
            unknown_count += 1

    recognized_students = list(best_matches.keys())
    confidence_scores = [round(best_matches[sid], 4) for sid in recognized_students]

    # Absent = all class students not recognized
    ...
    # Resolve names
    recognized_names = [student_map[sid]["name"] for sid in recognized_students if sid in student_map]
    absent_names = [student_map[sid]["name"] for sid in absent_students if sid in student_map]
